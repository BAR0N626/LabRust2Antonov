...
fn run() -> AppResult<()> {
    ...
    let mut prepared_inputs: Vec<PreparedInput> = Vec::new();
    ...
    for (index, line) in content.lines().enumerate() {
        ...
        match read_source_bytes(&client, source) {
            Ok(bytes) => {
    ...
    let processed_results: Vec<Result<ProcessedImage, Box<AppError>>> = prepared_inputs
        .into_par_iter()
        .map(|item| process_prepared_image(item, width, height))
        .collect();
    ...
    for result in processed_results {
        match result {
            Ok(image) => {
                if let Err(err) = uploader.upload(image.bytes, &image.filename) {
                    eprintln!("Рядок {}: {} -> {}", image.index, image.source, err);
                    had_errors = true;
                }
            }
            Err(err) => {
...
fn read_source_bytes(client: &Client, source: &str) -> AppResult<Vec<u8>> {
    if is_url(source) {
...
/// Виконує CPU-bound обробку зображення в окремому потоці rayon.
///
/// # Errors
///
/// Повертає помилку, якщо:
