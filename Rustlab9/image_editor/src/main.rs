...
/// Точка входу у програму з ручним налаштуванням рантайму.
fn main() {
    let worker_threads = match env::var("MYME_RUNTIME_THREADS") {
        Ok(value) => value.parse::<usize>().unwrap_or(4),
        Err(_) => 4,
    };
    ...
    let runtime = Builder::new_multi_thread()
        ...
        .enable_all()
        .build()
        .unwrap_or_else(|e| panic!("Не вдалося створити Tokio runtime: {}", e));
    ...
    runtime.block_on(async move {
        ...
        match run().await {
            Ok(()) => {
                println!("Total program elapsed: {:.3?}", full_start.elapsed());
            }
            Err(error) => {
                eprintln!("Помилка: {}", error);
                eprintln!("Total program elapsed: {:.3?}", full_start.elapsed());
                std::process::exit(1);
            }
        }
    });
...
async fn run() -> AppResult<()> {
    ...
    for (index, line) in content.lines().enumerate() {
        ...
        read_tasks.push(async move {
            match read_source_bytes(&client, &source).await {
                Ok(bytes) => Ok(PreparedInput {
                    index: index + 1,
                    source,
                    bytes,
                }),
                Err(err) => Err((index + 1, source, err)),
            }
        });
    }
    ...
    while let Some(result) = read_tasks.next().await {
        match result {
            Ok(item) => prepared_inputs.push(item),
            Err((index, source, err)) => {
    ...
    let mut cpu_set = JoinSet::new();

    for item in prepared_inputs {
        cpu_set.spawn_blocking(move || process_prepared_image(item, width, height));
    }

    let mut processed_images = Vec::new();

    while let Some(task_result) = cpu_set.join_next().await {
        match task_result {
            ...
            Err(err) => {
                return Err(Box::new(AppError::Join(err.to_string())));
            }
        }
    }
    ...
    println!("Read stage elapsed: {:.3?}", read_elapsed);
    println!("CPU-bound stage elapsed: {:.3?}", cpu_elapsed);
    println!("Upload stage elapsed: {:.3?}", upload_elapsed);
